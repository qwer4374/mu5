...
class InstagramDownloader(PlatformDownloader):
    ...
    def extract_info(self, url: str, process: bool = False) -> dict:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            print("[yt-dlp INFO][Instagram] extract_info info_dict:", json.dumps(info, ensure_ascii=False, indent=2))
            return info
    ...
    def download(self, url: str, audio_only: bool = False):
        try:
            ...
            print(f"[yt-dlp DEBUG][Instagram] outtmpl: {outtmpl}")
            ydl_opts = {
                'outtmpl': outtmpl,
                'quiet': True,
            ...
            print(f"[DEBUG][Instagram] Files before: {before_files}")
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ydl.download([url])
            after_files = set(glob(os.path.join(abs_download_dir, '*.mp4')))
            print(f"[DEBUG][Instagram] Files after: {after_files}")
            new_files = after_files - before_files
            print(f"[DEBUG][Instagram] New files: {new_files}")
            import glob as _glob
            all_mp4_files = set(_glob.glob(os.path.join(abs_download_dir, '**', '*.mp4'), recursive=True))
            print(f"[DEBUG][Instagram] All mp4 files in downloads/: {all_mp4_files}")
            mp4_files = list(glob(os.path.join(abs_download_dir, '*.mp4')))
            if not mp4_files:
                print("[yt-dlp ERROR][Instagram] No mp4 files found after download!")
                return None
            latest_file = max(mp4_files, key=os.path.getmtime)
            print(f"[yt-dlp DEBUG][Instagram] Latest mp4 file after download: {latest_file}")
            ...
            print(f"[yt-dlp ERROR][Instagram] Unexpected exception in download: {e}")
            return None
